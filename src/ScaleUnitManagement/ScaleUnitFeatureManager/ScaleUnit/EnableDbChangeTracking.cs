...
namespace ScaleUnitManagement.ScaleUnitFeatureManager.ScaleUnit
{
    public class EnableDbChangeTracking : IScaleUnitStep
    {
        ...
        public  void Run()
        {
            string sqlQuery = $"USE master; IF NOT EXISTS (SELECT * FROM sys.change_tracking_databases WHERE database_id=DB_ID('{Config.AxScaleUnitDbName()}')) ALTER DATABASE {Config.AxDbName()} SET CHANGE_TRACKING = ON(CHANGE_RETENTION = 2 DAYS, AUTO_CLEANUP = ON)";
